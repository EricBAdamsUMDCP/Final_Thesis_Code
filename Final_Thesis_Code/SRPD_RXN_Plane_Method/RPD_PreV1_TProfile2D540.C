...
void RPD_PreV1_TProfile2D540(int runnumber, int EtaBins ,int PtBins, int numSystematics, int numVertex, int numEff){
	...
	for (Int_t i=0;i<nCent;i++){		
		...
		//V1Even->cd();
		V1_Even_Histo[i] = new TH1D(Form("RPD_V1_Even_%s", centralitystring[i].c_str()),Form("RPD_V1_Even_5-40%;  Pt; V1 Magnitude [A.U.]", centralitystring[i].c_str()), PtBins, Ptmin, Ptmax);
	
		//V1Odd->cd();
		V1_Odd_Histo[i] = new TH1D(Form("RPD_V1_Odd_%s", centralitystring[i].c_str()), Form("RPD_V1_Odd_5-40%; Eta; V1 Magnitude [A.U.]", centralitystring[i].c_str()), EtaBins, etamin, etamax);
			...
			for (Int_t ii=0;ii<AcceptedNumberOfTracks;ii++){
			   ...
			   if(fabs(eta_Value)<=1.6 &&Pt_Value>0.4){
				   ...
				   PtvsCentrality->Fill(C+0.2, Pt_Value);
			   	   ...
			   	   Res->Fill(C, (TMath::Cos(EPfinalhfodd - EPfinalhfeven))); ///still needs sqrt to be real res
	...
	for (int q = 1; q < 10; q++){
		...
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Neg_NumTrk_wo_RXN_vs_Cent_540.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		TP_Pos_NumTrk_wo_RXN_vs_Cent->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Pos_NumTrk_wo_RXN_vs_Cent_540.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		TP_Pos_NumTrk_wo_RXN_vs_NTrack->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Pos_NumTrk_wo_RXN_vs_NTrack_540.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		TP_Neg_NumTrk_wo_RXN_vs_NTrack->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Neg_NumTrk_wo_RXN_vs_NTrack_540.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		TP_Cent_vs_NTrack->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Cent_vs_NTrack_540.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		PtvsCentrality->Draw("COLZ");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_PtvsCentrality_540.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		
		CompareRXN->Draw("COLZ");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_compare_all_cent__540.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		V1_Even_Histo[q-1]->Draw("HIST E0");
		V1_Even_Histo[q-1]->SetMarkerStyle(20);
		V1_Even_Histo[q-1]->SetMarkerSize(3);
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_V1_Even_Histo_5-40_%s.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber, centralitystring[q-1].c_str()));
		V1_Even_Histo[q-1]->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/ROOT/%s/%s/%d_V1_Even_Histo_5-40_%s.root", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber, centralitystring[q-1].c_str()));
		V1_Odd_Histo[q-1]->Draw("HIST E0"); 
		V1_Odd_Histo[q-1]->SetMarkerStyle(20);
		V1_Odd_Histo[q-1]->SetMarkerSize(3);
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_V1_Odd_Histo_5-40_%s.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber,centralitystring[q-1].c_str()));
		V1_Odd_Histo[q-1]->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/ROOT/%s/%s/%d_V1_Odd_Histo_5-40_%s.root", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber, centralitystring[q-1].c_str()));
