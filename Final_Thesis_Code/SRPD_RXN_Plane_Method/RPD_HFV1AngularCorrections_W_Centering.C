...
//////////////////////////////////////////////////////
Int_t RPD_HFV1AngularCorrections_W_Centering(int runnumber, int numSystematics){
...
void Initialize(int runnumber, string Systematics){
  ...
  //Create the output ROOT file
  myFile = new TFile(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/Sys_%s_RPD_RN_%d_JaimeFourierCode.root",runnumber, Systematics.c_str(), runnumber),"recreate");
  //Make Subdirectories for what will follow
...
/*    t->SetBranchAddress("HFPos_Phi", &hfp_phi_pos);
    ...
    ofstream myout;
    cout << "test\n";
    myout.open(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/Sys_%s_RN_%d_CosAndSinValues.txt", runnumber, Systematics.c_str(), runnumber)); 
