...
void PreV1_TProfile2D580(int runnumber, int EtaBins ,int PtBins, int numSystematics, int numVertex, int numEff){
  ...
  double WeightValue = 1;
	...
	for (Int_t i=0;i<nCent;i++){		
		...
		//V1Even->cd();
		V1_Even_Histo[i] = new TH1D(Form("HF_V1_Even_%s", centralitystring[i].c_str()),Form("HF_V1_Even_5-80;  Pt; V1 Magnitude [A.U.]", centralitystring[i].c_str()), PtBins, Ptmin, Ptmax);
	
		//V1Odd->cd();
		V1_Odd_Histo[i] = new TH1D(Form("HF_V1_Odd_%s", centralitystring[i].c_str()), Form("HF_V1_Odd_50-80; Eta; V1 Magnitude [A.U.]", centralitystring[i].c_str()), EtaBins, etamin, etamax);
...
/*		V1PtOdd[i]->SetMaximum(0.02);
	...
	for (int q = 1; q < 10; q++){
		...
		D->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Neg_NumTrk_wo_RXN_vs_Cent_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		TP_Pos_NumTrk_wo_RXN_vs_Cent->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Pos_NumTrk_wo_RXN_vs_Cent_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		TP_Pos_NumTrk_wo_RXN_vs_NTrack->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Pos_NumTrk_wo_RXN_vs_NTrack_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		TP_Neg_NumTrk_wo_RXN_vs_NTrack->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Neg_NumTrk_wo_RXN_vs_NTrack_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		TP_Cent_vs_NTrack->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Cent_vs_NTrack_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		PtvsCentrality->Draw("COLZ");
		D->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/PNG/%s/%s/%d_PtvsCentrality_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		CompareRXN->Draw("COLZ");
		D->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/PNG/%s/%s/%d_compare_all_cent__580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		V1_Even_Histo[q-1]->Draw("HIST E");
		V1_Even_Histo[q-1]->SetMarkerStyle(20);
		V1_Even_Histo[q-1]->SetMarkerSize(3);
		D->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/PNG/%s/%s/%d_HF_V1_Even_Histo_5-80_%s.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber, centralitystring[q-1].c_str()));
		V1_Even_Histo[q-1]->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/ROOT/%s/%s/%d_HF_V1_Even_Histo_5-80_%s.root", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber, centralitystring[q-1].c_str()));
		V1_Odd_Histo[q-1]->Draw("HIST E"); 
		V1_Odd_Histo[q-1]->SetMarkerStyle(20);
		V1_Odd_Histo[q-1]->SetMarkerSize(3);
		D->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/PNG/%s/%s/%d_HF_V1_Odd_Histo_5-80_%s.png", runnumber, Systematics.c_str(), Vertex.c_str(),runnumber, centralitystring[q-1].c_str()));
		V1_Odd_Histo[q-1]->SaveAs(Form("/data2/users/ebadams/Eric_HF_RXN_Data_Sets/%d/ROOT/%s/%s/%d_HF_V1_Odd_Histo_5-80_%s.root", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber, centralitystring[q-1].c_str()));
