...
for RunNumber in range(len(content_array)): #generates the number of .sh necessary for all the given root files
	...
	pythonfileholder = ('root -l -b n ' + '"/home/ebadams/Final_Thesis_Code/HF_RXN_Plane_Method/' + str(nameofprogram) + ".C+(" + str(content_array[RunNumber]) + ", 7, 16, " + str(nameofsystematics) + ", " + str(nameofvertex) + ', 1)"')
