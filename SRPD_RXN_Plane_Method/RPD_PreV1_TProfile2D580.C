...
void RPD_PreV1_TProfile2D580(int runnumber, int EtaBins ,int PtBins, int numSystematics, int numVertex, int numEff){
	...
	//resolutions as made by jaimes code from the ptstats made by me
	// HFV1PTStats.C
	//HFV1EPPlotting.C
	...
	TFile *f = new TFile(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/Sys_%s_RN_%d_RPD_Fourier_Flattened_w_centering_RXN_Plane_Tree_and_Tracks_HF_JAIME.root",runnumber, Systematics.c_str(), runnumber));
    //std::cout<<f<<std::endl;
	...
	TFile *myFile;
	TDirectory* myPlots;
	myFile = new TFile(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/Sys_%s_RN_%d_RPD_EPPlottingV1.root", runnumber, Systematics.c_str(), runnumber),"recreate");
	...
	for (Int_t i=0;i<nCent;i++){		
		...
		//V1Even->cd();
		V1_Even_Histo[i] = new TH1D(Form("RPD_V1_Even_%s", centralitystring[i].c_str()),Form("RPD_V1_Even_5-80%;  Pt; V1 Magnitude [A.U.]", centralitystring[i].c_str()), PtBins, Ptmin, Ptmax);
	
		//V1Odd->cd();
		V1_Odd_Histo[i] = new TH1D(Form("RPD_V1_Odd_%s", centralitystring[i].c_str()), Form("RPD_V1_Odd_5-80%; Eta; V1 Magnitude [A.U.]", centralitystring[i].c_str()), EtaBins, etamin, etamax);
	...
	FinalRes = new TProfile("RES",Form("V1_Resolution_RPD_%d; 10 Percent Centrality Bins; #sqrt{<cos(#Psi_{1}^{RPD^{+}} - #Psi_{1}^{RPD^{-}})>}",runnumber),10,0,9);
			...
			for (Int_t ii=0;ii<AcceptedNumberOfTracks;ii++){
			   ...
			   if(fabs(eta_Value)<=1.6 &&Pt_Value>0.4){
				   ...
				   PtvsCentrality->Fill(C+0.2, Pt_Value);
			   	   ...
			   	   Res->Fill(C, (TMath::Cos(EPfinalhfodd - EPfinalhfeven)*WeightValue)); ///still needs sqrt to be real res
			       V1EtaOdd[C]->Fill(eta_Value,(TMath::Cos(phi_Value-EPfinalhfodd)*WeightValue));
			       V1EtaEven[C]->Fill(eta_Value,(TMath::Cos(phi_Value-EPfinalhfeven)*WeightValue));
			   	   //V1Odd[C]->Fill(eta_Value,((TMath::Cos(phi_Value-EPfinalhfodd) + TMath::Cos(phi_Value-EPfinalhfeven)))/2/*/OddResolution[C]*/);
			   	  

			       V1PtEven[C]->Fill(Pt_Value,(TMath::Cos(phi_Value-EPfinalhfeven)*WeightValue));
			       V1PtOdd[C]->Fill(Pt_Value,(TMath::Cos(phi_Value-EPfinalhfodd)*WeightValue));//Can find offset later with removing the eta gate here
			       //V1Even[C]->Fill(Pt_Value,(TMath::Cos(phi_Value-EPfinalhfeven) + -1*TMath::Cos(phi_Value-EPfinalhfodd))/2/*/EvenResolution[C]*/);
	...
	for (int q = 1; q < 10; q++){
		...
		for (int e = 0; e < EtaBins; e++){
			...
			V1Odd[q-1]->Fill(EtaRanges[e],(/*0.85**/BinContentOeta + BinContentEeta)/(2*BinContent));
			V1_Odd_Histo[q-1]->Fill(EtaRanges[e],(/*0.85**/BinContentOeta + BinContentEeta)/(2*BinContent)); // something hee is very right
		...
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Neg_NumTrk_wo_RXN_vs_Cent_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		TP_Pos_NumTrk_wo_RXN_vs_Cent->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Pos_NumTrk_wo_RXN_vs_Cent_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		TP_Pos_NumTrk_wo_RXN_vs_NTrack->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Pos_NumTrk_wo_RXN_vs_NTrack_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		TP_Neg_NumTrk_wo_RXN_vs_NTrack->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Neg_NumTrk_wo_RXN_vs_NTrack_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		TP_Cent_vs_NTrack->Draw("HIST E");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_TP_Cent_vs_NTrack_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));

		PtvsCentrality->Draw("COLZ");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_PtvsCentrality_580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		
		CompareRXN->Draw("COLZ");
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_compare_all_cent__580.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber));
		V1_Even_Histo[q-1]->Draw("HIST E0");
		V1_Even_Histo[q-1]->SetMarkerStyle(20);
		V1_Even_Histo[q-1]->SetMarkerSize(3);
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_V1_Even_Histo_5-80_%s.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber, centralitystring[q-1].c_str()));
		V1_Even_Histo[q-1]->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/ROOT/%s/%s/%d_V1_Even_Histo_5-80_%s.root", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber, centralitystring[q-1].c_str()));
		V1_Odd_Histo[q-1]->Draw("HIST E0"); 
		V1_Odd_Histo[q-1]->SetMarkerStyle(20);
		V1_Odd_Histo[q-1]->SetMarkerSize(3);
		D->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/PNG/%s/%s/%d_V1_Odd_Histo_5-80_%s.png", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber,centralitystring[q-1].c_str()));
		V1_Odd_Histo[q-1]->SaveAs(Form("/data2/users/ebadams/Eric_RPD_RXN_Data_Sets/%d/ROOT/%s/%s/%d_V1_Odd_Histo_5-80_%s.root", runnumber, Systematics.c_str(), Vertex.c_str(), runnumber, centralitystring[q-1].c_str()));
